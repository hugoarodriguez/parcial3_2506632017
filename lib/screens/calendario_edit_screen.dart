...
class CalendarioEditScreen extends StatelessWidget {
  const CalendarioEditScreen({Key? key}) : super(key: key);
...
class _CalendarioEditForm extends StatefulWidget {

  @override
  State<_CalendarioEditForm> createState() => _CalendarioEditFormState();
}

class _CalendarioEditFormState extends State<_CalendarioEditForm> {
 ...
 //Controladores
  ...
  Widget build(BuildContext context) {
    ...
    final idCalendario = ModalRoute.of(context)!.settings.arguments;

    return FutureBuilder(
      future: calendariosProvider.getCalendario(idCalendario.toString()),
      builder: (context, AsyncSnapshot<List<CalendarioModel>> snapshot){
        ...
        if(snapshot.hasData){
          var datosCalendario = snapshot.data;

          _nombre.text = datosCalendario![0].nombre;
          _detalles.text = datosCalendario[0].detalles;
          valorDropDownTipo = datosCalendario[0].tipo;
          _fecha_ini.text = datosCalendario[0].fecha_ini;
          _fecha_fin.text = datosCalendario[0].fecha_fin;
          _hora_ini.text = datosCalendario[0].hora_ini;
          _hora_fin.text = datosCalendario[0].hora_fin;
          _idusuario.text = datosCalendario[0].idusuario.toString();
          _img_ini.text = datosCalendario[0].img_ini;
          _img_fin.text = datosCalendario[0].img_fin;

          return mainContent(idCalendario.toString());
        }
  ...
  Widget mainContent(String idCalendario){
    return Card(
            ...
            MaterialButton(
              ...
              onPressed: () async {
                ...
                if(_formKey.currentState!.validate()){
                  bool r = await calendariosProvider.actCalendario(idCalendario, _detalles.text, _fecha_fin.text, _fecha_ini.text, _hora_fin.text, _hora_ini.text, int.parse(_idusuario.text), 
                  _img_fin.text, _img_ini.text, _nombre.text, _tipo);

                  if(r){
                    //Si se guardó el calendario
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('Actualizando calendario'))
                    );
